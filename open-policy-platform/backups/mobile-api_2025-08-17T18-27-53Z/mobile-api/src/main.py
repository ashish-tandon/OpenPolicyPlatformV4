...
def create_mobile_user(user_data: MobileUserCreate):
    """Create a new mobile user"""
    start_time = time.time()
    
    try:
        user = mobile_service.create_user(user_data.dict())
...
def update_mobile_user(user_id: str, update_data: MobileUserUpdate):
    """Update mobile user information"""
    try:
        user = mobile_service.update_user(user_id, update_data.dict(exclude_unset=True))
        if not user:
            raise HTTPException(status_code=404, detail="User not found")
